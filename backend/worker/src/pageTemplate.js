...
/**
 * Build a new service page HTML from a template file.
 * Keeps layout/CSS; swaps identity strings + optional hero image.
 * Pass opts.siteOrigin (https://example.com) for absolute canonical/OG URLs.
 */
export function buildServicePageHtml(templateHtml, opts) {
  const title = String(opts.title || "New Page").trim();
  const slug = String(opts.slug || slugifyTitle(title));
  const path = `services/${slug}.html`;
  const description = String(
    opts.description || `${title} — add your short description here.`
  ).trim();
  const imageUrl = String(opts.imageUrl || "").trim();
  const siteOrigin = String(opts.siteOrigin || "").replace(/\/$/, "");
  ...
  const canonical = siteOrigin ? `${siteOrigin}/${cleanPath}` : `/${cleanPath}`;
...
/**
 * Append custom pages into Services-style dropdown panels (nav-drop-panel).
 */
export function injectCustomServiceNav(html, customPages, pagePath) {
  ...
  return String(html).replace(
    ...
    (full, open, inner, close) => {
      if (!/class="nav-drop-item/i.test(inner)) return full;
...
/**
 * Append simple cards on a services index rail for custom pages.
 */
export function injectCustomServiceCards(html, customPages, pagePath) {
  ...
  for (const page of list) {
    ...
    const desc = escapeHtml(
      page.description ||
        `${page.title || "This page"} — edit this page in the CMS.`
    );
    const img = escapeHtml(page.imageUrl || "");
    const num = String(n).padStart(2, "0");
    const imgTag = img
      ? `<img src="${img}" alt="${title}" class="w-full object-cover object-center" width="1600" height="900" loading="lazy" decoding="async" />`
      : "";
    cards.push(`
