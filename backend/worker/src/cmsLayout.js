...
export function normalizeMenuHref(href) {
  ...
  try {
    ...
    const url = new URL(abs, "https://cms.local/");
...
/** Split hash into data-cms-section so hover never shows #section. */
export function splitMenuHref(href) {
  ...
  try {
    const url = new URL(full, "https://cms.local/");
...
function anchorAttrs(item) {
  ...
  const section = parts.section
    ? ` data-cms-section="${escapeHtml(parts.section)}"`
    : "";
