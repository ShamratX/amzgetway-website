...
if(!class_exists("TX_Update_Base")) {
        ...
        public $key = "3DEA9A6E7243AD66";
        private $product_id = "1";
        private $product_base = "avas";
        private $server_host = "https://theme-x.org/wp-json/licensor/";
        ...
        private static $_onDeleteLicense=[];
            ...
            if(strpos($dir,'wp-content/themes')!==FALSE){
                $this->isTheme=true;
                        ...
                        // Defer loading version until after WordPress is initialized
                        add_action('after_setup_theme', [$this, 'initialize']);
                        
                        if($this->hasCheckUpdate) {
                                if(function_exists("add_action")){
                                        add_action( 'admin_post_avas_fupc', function(){
                                        ...
                                        if($this->isTheme){
                                                ...
                                                add_filter( 'plugin_row_meta', function($links, $plugin_file ){
                                                        if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
                                                                $links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=avas_fupc' ) . "'>Update Check</a>";
                                                        }
                                                        return $links;
                                                }, 10, 2 );
        ...
        private function removeOldWPResponse() {
            ...
            final function _CheckWPPlugin( $purchase_key, &$error = "", &$responseObj = null ) {
                        ...
                        if(in_array( $_SERVER['REMOTE_ADDR'], $whitelist)):
                                ...
                                $responseObj->license_title = 'Localhost (As your server is detected as localhost so you don\'t need to activate the license.)';
                                return true;
                        endif;
